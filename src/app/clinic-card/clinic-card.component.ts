...
import { ClinicService, LocationService } from '../service';
...
export class ClinicCardComponent implements OnInit {
  ...
  constructor(private clinicService: ClinicService, private modal: NgbActiveModal, private locationService: LocationService) { }

  ngOnInit(): void {
  }

  fillClinic(clinicId: number) {
  	this.clinicId = clinicId;


  	this.clinicService.getClinic(clinicId).subscribe(data => {
  		data = data['data'][0]
  		this.clinicName = data['name'];
  		this.clinicGrade = data['clinicGrade'];
  		this.clinicDescription = data['description'];
  		this.patientGrade = data['patientGrade'];

      this.locationService.getLocationDetails(data['address']).subscribe(result => {
        ...
        if(result.length != 1) {
          document.getElementById('mapa-klinike').style.height = '0px';
        ...
        this.map.addLayer(vector);
  		...
  		console.error('error while loading clinic');
  	});
  }

  rateClinic(grade): void {
